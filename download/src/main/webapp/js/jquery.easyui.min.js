...
},beforePageText:"当前第 ",afterPageText:" 页,共 {pages} 页",displayMsg:"第 {from} 到 {to} 条数据,共 {total} 条数据"};
})(jQuery);
(function($){
function _93(_94){
var _95=$(_94);
...
/*_2a6.options.onContextMenu.call(_2a4,e,$(this).find("span.tabs-title").html(),_2a7(_2a4,e.data.p));*/
	...
	menuDiv.id=_2a4.id+"_contextMenu";
	menuDiv.style.width=100;
	document.body.appendChild(menuDiv);
	
	var item1=document.createElement("div");
	item1.innerHTML="关闭其他tab页";//"Close current";
	//menuDiv.appendChild(item1);
	var item2=document.createElement("div");
	item2.innerHTML="关闭所有tab页";
	menuDiv.appendChild(item2);
	$('#'+_2a4.id+"_contextMenu").menu({   
	    onClick:function(item){   
	    	if(item.text=='关闭其他tab页'){
	    		var tab = $(_2a4).tabs('getSelected');
				var index = $(_2a4).tabs('getTabIndex',tab);
				var tabs=$(_2a4).tabs('tabs'); 
				for(var i=0;i<tabs.length;i++){
		    		if($(tabs[i]).panel('options').closable){
		    			var index2=$(_2a4).tabs('getTabIndex',tabs[i]);
		    			if(index2!=index){
		    				$(_2a4).tabs('close',index);
		    			}
		    			i--;
		    		}
		    	}
	    	}else if(item.text=='关闭所有tab页'){
	    		var tabs=$(_2a4).tabs('tabs'); 
		    	for(var i=0;i<tabs.length;i++){
		    		if($(tabs[i]).panel('options').closable){
		    			var index=$(_2a4).tabs('getTabIndex',tabs[i]);
		    			$(_2a4).tabs('close',index);
		    			i--;
		    		}
		    	}
	...
	$('#'+_2a4.id+"_contextMenu").menu('show', {   
	  left: e.clientX,   
	  top: e.clientY   
	});
});
...
$.fn.datagrid.defaults=$.extend({},$.fn.panel.defaults,{frozenColumns:undefined,columns:undefined,fitColumns:false,autoRowHeight:true,toolbar:null,striped:false,method:"post",nowrap:true,idField:null,url:null,loadMsg:"正在加载 ...",rownumbers:false,singleSelect:false,selectOnCheck:true,checkOnSelect:true,pagination:false,pagePosition:"bottom",pageNumber:1,pageSize:10,pageList:[10,20,30,40,50],queryParams:{},sortName:null,sortOrder:"asc",remoteSort:true,showHeader:true,showFooter:false,scrollbarSize:18,rowStyler:function(_5e5,_5e6){
